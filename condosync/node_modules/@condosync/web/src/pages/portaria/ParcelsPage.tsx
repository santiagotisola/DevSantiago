...
export function ParcelsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/parcels/condominium/${selectedCondominiumId}`,
        { params: { limit: 50 } },
      );
  ...
  const { data: residents, isLoading: isLoadingResidents } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get(
        `/residents/condominium/${selectedCondominiumId}`,
      );
  ...
  const activeParcel = pickupModal
    ? (data?.parcels || []).find((p: any) => p.id === pickupModal)
    : null;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Registro e controle de encomendas recebidas
          </p>
        </div>
        {canRegister && (
          <button
                ...
                {parcels.map((p: any) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_LABELS[p.status]?.color}`}
                      >
              ...
              <div className="space-y-1">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const resident = pickupOptions.find(
                      (r: any) => r.id === residentId,
                    );
                ...
                >
                  ...
                  {pickupOptions.map((r: any) => (
                    <option key={r.id} value={r.id}>
                      {r.user?.name}
                      {r.unit?.identifier ? ` - ${r.unit.identifier}` : ""}
                    </option>
                  ))}
                </select>
                <p className="text-xs text-muted-foreground">
                  Moradores da unidade, ou de todo o condomínio quando a unidade
                  não possui morador vinculado.
                </p>
              </div>
            )}
            <div className="space-y-1">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl w-full max-w-sm p-6 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Confirma o cancelamento/devolução desta encomenda? Ela será
              marcada como "Devolvida" no histórico.
            </p>
            <div className="space-y-1">
