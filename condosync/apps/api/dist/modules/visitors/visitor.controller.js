...
class VisitorController {
    ...
    async authorize(req, res) {
        ...
        const visitor = await visitor_service_1.visitorService.authorize(req.params.id, req.user.userId, authorized);
        res.json({ success: true, data: { visitor } });
    }
    ...
    async historyByUnit(req, res) {
        const data = await visitor_service_1.visitorService.historyByUnit(req.params.unitId, Number(req.query.page) || 1, Number(req.query.limit) || 20);
        res.json({ success: true, data });
    }
}
