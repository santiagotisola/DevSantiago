...
const finance_scheduler_1 = require("./modules/finance/finance.scheduler");
// Rotas
...
const panic_routes_1 = __importDefault(require("./modules/panic/panic.routes"));
const recurrence_routes_1 = __importDefault(require("./modules/visitors/recurrence.routes"));
const financeCategories_routes_1 = __importDefault(require("./modules/finance/financeCategories.routes"));
...
exports.io.on("connection", (socket) => {
    ...
    socket.on("join:user", (userId) => {
        if (!userId || userId !== socket.data.userId) {
            logger_1.logger.warn(`Socket ${socket.id} tentou entrar em room de outro usuÃƒÆ’Ã‚Â¡rio`);
            return;
        }
        socket.join(`user:${userId}`);
    });
    socket.on("join:condominium", async (condominiumId) => {
        ...
        if (!membership) {
            logger_1.logger.warn(`Socket ${socket.id} sem vÃƒÆ’Ã‚Â­nculo tentou entrar no condomÃƒÆ’Ã‚Â­nio ${condominiumId}`);
            return;
        }
        socket.join(`condominium:${condominiumId}`);
        if (["DOORMAN", "CONDOMINIUM_ADMIN", "SYNDIC", "SUPER_ADMIN"].includes(membership.role)) {
    ...
    socket.on("join:unit", async (unitId) => {
        ...
        if (!membership) {
            logger_1.logger.warn(`Socket ${socket.id} sem vÃƒÆ’Ã‚Â­nculo tentou entrar na unidade ${unitId}`);
            return;
        }
...
app.use(`${API}/panic`, panic_routes_1.default);
app.use(`${API}/visitor-recurrences`, recurrence_routes_1.default);
app.use(`${API}/finance-categories`, financeCategories_routes_1.default);
...
httpServer.listen(PORT, async () => {
    logger_1.logger.info(`ÃƒÂ°Ã…Â¸Ã…Â¡Ã¢â€šÂ¬ CondoSync API rodando na porta ${PORT}`);
    logger_1.logger.info(`ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã¢â‚¬Â¹ Ambiente: ${env_1.env.NODE_ENV}`);
    logger_1.logger.info(`ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â URL: http://localhost:${PORT}`);
    await (0, maintenance_alerts_worker_1.registerMaintenanceAlertsSchedule)();
    await (0, finance_scheduler_1.registerFinanceSchedule)();
});
exports.default = app;
